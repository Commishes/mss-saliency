...
///	RGB2LAB
//===========================================================================
void SymmetricSurroundSaliency::RGB2LAB(const vector<RGBColour>& ubuff,
		vector<double>& lvec, vector<double>& avec, vector<double>& bvec)
{
...
//==============================================================================
void SymmetricSurroundSaliency::GaussianSmooth(const vector<double>& inputImg,
		const int& width, const int& height, const vector<double>& kernel,
		...
		for (int r = 0; r < rows; r++)
		{
			for (int c = 0; c < cols; c++)
			{
				...
				for (int cc = (-center); cc <= center; cc++)
				{
					if (((c + cc) >= 0) && ((c + cc) < cols))
					{
						sum += inputImg[r * cols + (c + cc)] * kernel[center
								+ cc];
...
void SymmetricSurroundSaliency::ComputeMaximumSymmetricSurroundSaliency(
		const vector<RGBColour>& img, const int& width, const int& height,
		vector<double>& salmap, const bool& normflag)
{
	int sz = width * height;
	salmap.resize(sz);
	...
	for (int j = 0; j < height; j++)
	{
		...
		for (int k = 0; k < width; k++)
		{
			...
			salmap[ind] = (lval - ls[ind]) * (lval - ls[ind])
					+ (aval - as[ind]) * (aval - as[ind]) + (bval - bs[ind])
					* (bval - bs[ind]);//square of the euclidean distance
			//------
			ind++;
			//------
		}
